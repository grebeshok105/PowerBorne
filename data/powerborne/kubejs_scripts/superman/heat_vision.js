PlayerEvents.tick(event => {
    ...
    if (abilityUtil.isEnabled(player, "powerborne:superman", "heat_vision_beam")) {
        ...
        let currentTick = global.getAbilityAnimationTicks(player, "powerborne:superman", "heat_vision_timer", true);
        let relativeMax = 60;
        ...
        if (block.id === "minecraft:fire") {
            ...
            if (destroySpeed <= 0.1) {
                ...
                pd.remove("heat_vision_progress");
            } else {
                let totalHeatNeeded = Math.max(20, destroySpeed * 20) / heatVisionSpeedMultiplier;
                let heatProgress = pd.getInt("heat_vision_progress") || 0;
                heatProgress++;
                ...
                if (heatProgress >= totalHeatNeeded) {
                    ...
                    pd.putInt("heat_vision_progress", heatProgress);
                }
            }
        } else if (currentTick > 5) {
            lightUpBlock(player, block, blockPos);
...
function lightUpBlock(player, block, blockPos) {
